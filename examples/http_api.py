...
class LinkedInWrapper(object):
    ...
    def __init__(self, id, secret, port):
        ...
        print("Please double check that the callback URL has been correctly added in the developer console: https://www.linkedin.com/developer/apps/ \n\n")
...
class CustomHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsedurl.path in ['/code/', '/code']:
            ...
            lkin_api.authentication.authorization_code = params_to_d(self.path).get('code')
            self.wfile.write(dumps({'access_token': lkin_api.authentication.get_access_token(),
                                    'routes': list([d for d in dir(lkin_api.application) if not d.startswith('_')])}).encode('utf8'))
        elif parsedurl.path == '/routes':
            self.json_headers()

            self.wfile.write(dumps({'routes': list([d for d in dir(lkin_api.application) if not d.startswith('_')])}).encode('utf8'))
        elif not authed:
            self.json_headers()
            ...
            self.wfile.write(dumps({'path': self.path, 'authed': type(lkin_api.authentication.token) is None}).encode('utf8'))
        elif authed and len(parsedurl.path) and parsedurl.path[1:] in dir(lkin_api.application):
            self.json_headers()
            self.wfile.write(dumps(getattr(lkin_api.application, parsedurl.path[1:])()).encode('utf8'))
        else:
